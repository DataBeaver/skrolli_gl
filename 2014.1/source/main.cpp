...
int main()
{
	...
	camera.set_position(Vector(5, -23, 1.7));
	camera.set_heading(90);
	camera.set_depth_range(0.1, 100);

	Controller controller(camera);
	engine.set_event_listener(&controller);

	engine.set_background_color(0.40, 0.63, 0.75);
	engine.set_light_direction(Vector(-0.8, -0.7, 1.3));
	engine.set_light_intensity(0.7);
	engine.set_ambient_intensity(0.4);
